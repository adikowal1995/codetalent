...
describe('AboutSection', () => {
  ...
  it('renders the description text', () => {
    renderWithTheme(<AboutSection />)
    expect(screen.getByText(/Jesteśmy agencją rekrutacyjną/i)).toBeInTheDocument()
  })
  ...
  it('renders value descriptions', () => {
    renderWithTheme(<AboutSection />)
    expect(screen.getByText(/Fokusujemy się wyłącznie na rekrutacjach IT/i)).toBeInTheDocument()
    expect(screen.getByText(/Jasne procesy, czytelne stawki/i)).toBeInTheDocument()
    expect(screen.getByText(/Dokładna weryfikacja kandydatów/i)).toBeInTheDocument()
    expect(screen.getByText(/Długoterminowe relacje oparte na zaufaniu/i)).toBeInTheDocument()
  })
}) 